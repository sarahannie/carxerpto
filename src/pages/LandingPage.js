...
import { CustomSpinner } from "../loading.js";

function LandingPage() {
  // Assume these hooks represent various data fetching operations
  const { isLoading: isProductsLoading, error: productsError} = useGetProductsQuery();
  ...
  if (isLoading) {
    return (
      <div className="flex justify-center items-center min-h-screen">
        <div className="flex mx-auto justify-center items-center bg-white w-full">
          <CustomSpinner />
        </div>
      </div>
    );
  ...
  if (error) {
    return <p>Logout and login again</p>;
  }
          ...
          <div className="flex lg:flex-row flex-col px-4 lg:mt-8 text-center lg:justify-start lg:items-start justify-center items-center mt-16">
            <div className="flex flex-col lg:justify-start lg:items-start lg:w-[40%] pr-8 w-[100%] justify-center items-center">
              ...
              <div className="mt-6 space-x-4 flex gap-1 ml-3">
                <Button className="bg-primary-normal text-white lg:px-[50px] px-[10px]">
                  <Link to="/car-listing">Buy Your Car</Link>
                </Button>
                <Button className="bg-transparent text-secondary-dark border-2 border-secondary-dark lg:px-[40px] px-[10px]">
                  <Link to="/car-broker">Meet an Xerpto</Link>
                </Button>
              </div>
            </div>
